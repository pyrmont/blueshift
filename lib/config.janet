      ...
      (cond
        ...
        (if (dict? el)
          (do
            ...
            (set el-i nil))
          (if (= 0 kl-i)
            (errorf "parse error on config.jdn, line %d: expected struct/table at top level" linenum)
            (errorf "parse error on config.jdn, line %d: expected struct/table to be mapped to keys %n" linenum (array/slice kl 0 kl-i))))
        (= :ind next-coll)
        (if (ind? el)
          (do
            (set next-coll nil)
            ...
            (set el-i 0))
          (if (= 0 kl-i)
            (errorf "parse error on config.jdn, line %d: expected struct/table at top level of config.jdn file")
            (errorf "parse error on config.jdn, line %d: expected struct/table to be mapped to keys %n" (array/slice kl 0 kl-i))))
        # default
        (errorf "parse error on config.jdn, line %d: error parsing config.jdn"))
      el-val?
      (if (val? el)
        (if (= (length kl) kl-i)
          (do
            ...
            (do
              ...
              (if (dict? el)
                ...
                (do
                  ...
                  (set el-i 0))))
            (break)))
        (errorf "parse error on config.jdn, line %d: struct/table must have even number of key-value pairs"))
      (= "}" el)
      (break)
      (= "]" el)
      (break)
      ...
      (if (val? el)
        (if (= el (describe (get kl kl-i)))
          ...
          (do
            (set el-key? false)
            (set el-val? false)))
        (error "parse error on config.jdn, line %d: expected key"))
      el-i
      (if (= el-i (get kl kl-i))
        (if (= (length kl) (++ kl-i))
          (do
    ...
    (do
      ...
      (if (dict? coll)
        (do
          ...
          (when (whitespace? (get coll (- key-i 1) "0"))
            (array/remove coll (- key-i 1))
            (when (whitespace? (get coll (- key-i 2) "0"))
              (array/remove coll (- key-i 2)))))
...
(defn upd-in
  ...
  (if pred
    ...
    (do
      (var val-i (last trail))
      (def coll (get-in ds (array/slice trail 0 -2)))
      (if (dict? coll)
        (swap coll val-i to indent)
        (error "not implemented for arrays/tuples"))))
  ds)
